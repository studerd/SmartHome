...
const bootstrap = async () => {
    ...
    app.setGlobalPrefix(_common_1.configManager.getValue(_common_1.ConfigKey.APP_BASE_URL));
    app.enableCors();
    const adapterHost = app.get(core_1.HttpAdapterHost);
    app.useGlobalFilters(new _common_1.HttpExceptionFilter(adapterHost));
    _common_1.swaggerConfiguration.config(app);
    await app.listen(_common_1.configManager.getValue(_common_1.ConfigKey.APP_PORT));
};
bootstrap().then(() => {
