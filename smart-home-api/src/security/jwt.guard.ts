...
import { Credential, NoTokenFoundedException, TokenExpiredException } from './data';

@Injectable()
export class JwtGuard implements CanActivate {
  private readonly logger = new Logger(JwtGuard.name);

  constructor(private readonly jwtService: JwtService, private readonly securityService: SecurityService, private reflector: Reflector) {
  }
  ...
  private validateToken(request: any): Observable<boolean> {
    if (!isNil(request.headers['authorization'])) {
      try {
        ...
        return from(this.securityService.detail(id)).pipe(map((user: Credential) => {
          request.user = user;
          return true;
        }));
      } catch (e) {
